...
static int m881340_phy_set_link_speed(void)
{
	...
	rv = miiphy_read(M2S_MII_NAME, M88E1111_PHY_ADDR, MII_ADVERTISE, &val);
	if (rv != 0) {
		goto out;
	}
	val |= (ADVERTISE_10HALF | ADVERTISE_10FULL
		| ADVERTISE_100HALF | ADVERTISE_100FULL);
	rv = miiphy_write(M2S_MII_NAME, M88E1111_PHY_ADDR, MII_ADVERTISE, val);
	if (rv != 0) {
		goto out;
	}

	/* Set 1000Mbps advertisement. */
	rv = miiphy_read(M2S_MII_NAME, M88E1111_PHY_ADDR, MII_CTRL1000, &val);
	if (rv != 0) {
		goto out;
	}

	val |= (ADVERTISE_1000FULL | ADVERTISE_1000HALF);
	rv = miiphy_write(M2S_MII_NAME, M88E1111_PHY_ADDR, MII_CTRL1000, val);
	if (rv != 0) {
		goto out;
	}
 out:
...
#if 0
static int m881111_phy_set_link_speed(void)
{
	u16 val;
	int rv;
        /* Set auto-negotiation advertisement. */
...
static int msgmii_phy_autonegotiate(void)
{
	...
	rv = miiphy_write(M2S_MII_NAME, M88E1111_PHY_ADDR,
			M88E1340_EXT_ADDR_PAGE_CR, PAGE_0);
	if (rv != 0) {
		goto out;
	}
	val = 0x9340 | PHY_BMCR_100_MBPS;
	rv = miiphy_write(M2S_MII_NAME, M88E1111_PHY_ADDR, PHY_BMCR, val);
	if (rv != 0) {
		goto out;
	}
	...
	while (timeout--) {
		rv = miiphy_read(M2S_MII_NAME, M88E1111_PHY_ADDR,
				PHY_BMSR, &val);
		if (rv != 0) {
			printf("mii err %d.\n", rv);
			goto out;
